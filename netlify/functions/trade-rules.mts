...
function note51HeadingForHts(index:Chapter99Index,hts:string):string|null{
  ...
  if(!note)return null;
  const d=digits(hts),parts=[
    {ref:"9903.03.12",start:/\(1\)\s*Heading\s+9903\.03\.12\s+applies/i,end:/\(2\)\s*Heading\s+9903\.03\.13\s+applies/i},
    {ref:"9903.03.13",start:/\(2\)\s*Heading\s+9903\.03\.13\s+applies/i,end:/\(3\)\s*Heading\s+9903\.03\.14\s+applies/i},
    {ref:"9903.03.14",start:/\(3\)\s*Heading\s+9903\.03\.14\s+applies/i,end:null}
  ];
  for(const part of parts){
    const sm=part.start.exec(note); if(!sm)continue;
    const start=(sm.index||0)+sm[0].length;
    const tail=note.slice(start);
...
function candidateRefs(index:Chapter99Index,hts:string,country:string){const refs=new Set<string>();for(const key of candidateKeys(hts))for(const ref of index.codes?.[key]||[]){if(!SECTION338_HEADINGS.has(ref))refs.add(ref);}if(country==="CA"){const s338=note51HeadingForHts(index,hts);if(s338)refs.add(s338);}return [...refs].sort();}
...
function programFor(ref:string,meta?:IndexHeading){const targets=meta?.noteTargets||[],context=headingContext(meta);if(targets.some(t=>/^51(?::|$)/.test(t)))return"Section 338";if(targets.some(t=>/^20(?::|$)/.test(t))||/section\s*301/i.test(context))return"Section 301";if(targets.some(t=>/^(16|33|38)(?::|$)/.test(t))||/section\s*232/i.test(context))return"Section 232";if(/section\s*201|safeguard/i.test(context))return"Section 201 / safeguard";if(/section\s*122/i.test(context))return"Section 122";if(targets.some(t=>/^30(?::|$)/.test(t))&&/russian federation/i.test(context))return"Russia Chapter 99 duty";return"Chapter 99";}
...
function applyVehicleSectoralStacking(measures:EvaluatedMeasure[],input:any){
  const vehicleClass=clean(fact(input,"vehiclePartCategory")).toLowerCase();
  if(!["mhd","passenger-light"].includes(vehicleClass))return;
  const vehicleCandidates=measures.filter(m=>m.applicability!=="not-applicable"&&(vehicleClass==="mhd"?m.noteTargets.some(t=>/^38:(?:h:)?[ij]$/.test(t)):m.noteTargets.some(t=>/^33:(?:g|j)$/.test(t))));
  if(!vehicleCandidates.length)return;
  const programLabel=vehicleClass==="mhd"?"medium-/heavy-duty vehicle-parts Section 232 treatment":"passenger-vehicle/light-truck parts Section 232 treatment";
  for(const m of measures){
    if(m.applicability==="not-applicable")continue;
    if(!m.noteTargets.some(t=>/^16(?::|$)/.test(t)))continue;
    m.applicability="not-applicable";
    m.estimatedDuty=null;
    m.replacementDutyEstimate=null;
    m.reason=`Not stacked with the confirmed ${programLabel}; the vehicle Section 232 stacking rules displace the separate sectoral metal duty for the same article.`;
  }
}
...
function applyWorstCaseEstimates(measures:EvaluatedMeasure[],input:any,customsValue:number,country:string){
  ...
  for(const m of measures){
    ...
    const vehicleUseFacts=new Set(["vehiclePartCategory","mhdProductionRepairCertification"]);
    if(m.applicability!=="applicable"&&m.requiredFacts.some(f=>vehicleUseFacts.has(f))){
      a.worstCaseEstimatedDuty=null;
      a.assumptions.push("Vehicle-use eligibility has not been established, so this potential vehicle duty is shown for review but is not added to the quick estimate.");
      continue;
    }
